import React from "react";

const Fis2Course = () => {
  return (
    <article
      className="post h-entry"
      itemScope
      itemType="http://schema.org/BlogPosting"
    >
      {/* <header className="post-header">
        <h1 className="post-title p-name" itemprop="name headline">
          Física Geral 2
        </h1>
        <p className="post-meta">
          <time
            className="dt-published"
            datetime="2024-04-16T00:00:00+00:00"
            itemprop="datePublished"
          >
            Apr 16, 2024
          </time>
        </p>
      </header> */}

      <div className="post-content e-content" itemProp="articleBody">
        <meta charSet="UTF-8" />
        <meta httpEquiv="X-UA-Compatible" content="IE=edge" />
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <title>Curso de Física 2</title>

        <link rel="stylesheet" href="/style.css" />

        <link rel="preconnect" href="https://fonts.googleapis.com" />
        <link
          rel="preconnect"
          href="https://fonts.gstatic.com"
          crossOrigin=""
        />
        <link
          href="https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900;1,100;1,300;1,400;1,500;1,700;1,900&amp;display=swap"
          rel="stylesheet"
        />

        <header>
          <h1>Curso de Física 2</h1>
          {/* <h3>Prof. José Américo Miranda</h3> */}
        </header>

        <hr />

        <section id="about">
          <h3>Departamento de Física – Universidade Federal de Pernambuco</h3>
          <p>
            Este material é o resultado do ensino da disciplina Física Geral 2,
            oferecida a futuros cientistas e engenheiros da Universidade Federal
            de Pernambuco (UFPE), nas oportunidades em que eu a lecionei nos
            últimos anos. As notas de aula seguem o cronograma dos capítulos 12
            a 20 do livro Fundamentos da Física, Gravitação, Ondas e
            Termodinâmica (Volume 2), de Halliday, Resnick e Walker (Editora
            LTD, 10ª edição, 2016). Porém, estas notas são o resultado da
            consulta a vários outros livros-texto (não deixe de consultar a
            vasta bibliografia complementar sugerida!) e alguns sites de cursos
            básicos de Física sobre os temas em questão, do Brasil e do
            exterior. Como esperado, as notas de aula também contêm minhas
            próprias reflexões sobre os diversos tópicos estudados e minha
            maneira pessoal de apresentá-los. Ressalto que este material de
            Física Geral 2 é uma simples coletânea de aulas, que ainda está
            “criando sua personalidade”. Portanto, está sob constante
            reconstrução e revisão. Também enfatizo que as notas de aula não têm
            a intenção de substituir, ou de ser equivalente a quaisquer livros
            texto. A leitura de livros (quantos mais, melhor!) continua sendo
            parte essencial do aprendizado e amadurecimento dos estudantes. Por
            outro lado, como tenho feito nos últimos anos, decidi tornar este
            material público, na esperança que possa ter alguma utilidade para
            estudantes e quem sabe, professores.
          </p>
        </section>

        <hr />

        <section className="list">
          <h3>Curso de Física Geral 2</h3>
          <ul>
            <li>
              Cronograma do curso
              <a
                href="https://drive.google.com/file/d/1xsxT_jr34I3GEKiJtV3dTr1yDc0TA9xa/view?usp=sharing"
                target="_blank"
                type="application/pdf"
              >
                (pdf)
              </a>
            </li>
            <li>
              Bibliografia complementar
              <a
                href="https://drive.google.com/file/d/1AXMAePk7RLEeEngeg3eRetzxBEGLwURy/view?usp=sharing"
                target="_blank"
                type="application/pdf"
              >
                (pdf)
              </a>
            </li>
            <li>Notas de aula</li>
            <ul>
              <li>
                Aula 01
                <a
                  href="https://drive.google.com/file/d/1Jhnq9IjkN9Y-hmzgBmDw3YLYJQtVIhMM/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 02
                <a
                  href="https://drive.google.com/file/d/11wm70JvtSsIjN9_1eiX6Pax9voHdD7KA/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 03
                <a
                  href="https://drive.google.com/file/d/1hoQgcM5zZzO9euctv_F4El-p9doqyzpp/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 04
                <a
                  href="https://drive.google.com/file/d/1hpRQDE8tA-8SjA-sB7tv3H_9uX7pGofT/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 05
                <a
                  href="https://drive.google.com/file/d/1mcSfO48ob5cdrGe6K4yiaUOkPy5RXQ3R/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 06
                <a
                  href="https://drive.google.com/file/d/1KjWZuRMjWycriv3YAoIdbqtkltLTm858/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 07
                <a
                  href="https://drive.google.com/file/d/1-geemyV4oMLNU6lftgbN1MgvvTbUZvTr/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 08
                <a
                  href="https://drive.google.com/file/d/1ugX2CQm6AEESM76MAqh0Tr3ta2rqgMEb/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 09
                <a
                  href="https://drive.google.com/file/d/137Olkd7esKFP1d5wG5ahHxVMfaeLh5sh/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 10
                <a
                  href="https://drive.google.com/file/d/1xhyezvbeIFvEIoTNnSEhBs15Hcq4ZOd5/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 11
                <a
                  href="https://drive.google.com/file/d/1KeKDq7iXAhy_IcmbORZTHw9pO-mPW9z-/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 12
                <a
                  href="https://drive.google.com/file/d/1PMoNDIYwDhMtzROfKgDjLlATrOi5kw2g/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 13
                <a
                  href="https://drive.google.com/file/d/120PvUHvmAztQvMBggC14ARBwoxF9xPcO/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 14
                <a
                  href="https://drive.google.com/file/d/1Gwywy3m_oDL-VlDpONZ7ZqSLINx_gzN2/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 15
                <a
                  href="https://drive.google.com/file/d/1c_Zf2ZJZQj6H2E9eg_Xpg9hHDjJLV6_Z/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 16
                <a
                  href="https://drive.google.com/file/d/1t3mIV93q_F-To0QU6BDfhVAgfmhI28la/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 17
                <a
                  href="https://drive.google.com/file/d/1bhBZOs8z6018MOBtpXz-TEMnq-tDwjsl/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 18
                <a
                  href="https://drive.google.com/file/d/18yoj72T85YMXrKpZWDF_RmBaFVV-K6hp/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 19
                <a
                  href="https://drive.google.com/file/d/18ph3mCK-W-YthnbZY0HGcd1edMfo4_R1/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 20
                <a
                  href="https://drive.google.com/file/d/1bQsuuHmnvvX6QRFguFRtHppu0XNhsk0u/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 21
                <a
                  href="https://drive.google.com/file/d/1EMIQcfeO4Gbi3plIyWT6VGAWkwdwhkRU/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 22
                <a
                  href="https://drive.google.com/file/d/14jPN6jucpJPs1kJE5jBEQzg08bt87GsX/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 23
                <a
                  href="https://drive.google.com/file/d/1dXmpXfzgIy80HpKH7YEGA4FQlmisiiKB/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 24
                <a
                  href="https://drive.google.com/file/d/1pIM5u1ONCbcZ0UO-AwRYrRaQU4KkbGvv/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 25
                <a
                  href="https://drive.google.com/file/d/1kzMAO_CDEI4ZLRFSUpgZTo49kCRFpn62/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 26
                <a
                  href="https://drive.google.com/file/d/1RvI0N_3wHE29TSRl5ckoBfebZcDW1lcc/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 27
                <a
                  href="https://drive.google.com/file/d/1rGbyATm-peixYRDFHGQ1glP21MuXm4uG/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Aula 28
                <a
                  href="https://drive.google.com/file/d/1BpOPE5FMxK1glXpH73s6v39b-8g0D0ZX/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>
            </ul>

            <li>Transparências powerpoint de algumas das aulas</li>
            <ul>
              <li>
                Aula 03
                <a
                  href="https://docs.google.com/presentation/d/1z7T0qXov07YTX7sB3E-QRAp7oAu9dR12/edit?usp=sharing&amp;ouid=117917823223915543509&amp;rtpof=true&amp;sd=true"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 04
                <a
                  href="https://drive.google.com/file/d/1GpRFIYFLDuKPN0OSpgjawe4KsUF6A3i0/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 05
                <a
                  href="https://drive.google.com/file/d/1uavlJPb6zwC4hA92lD58W-8zWn2CCvD_/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 06
                <a
                  href="https://drive.google.com/file/d/1R7mWClgDxN0eWHhjxfd4RcNfPCNbNFfW/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 07
                <a
                  href="https://docs.google.com/presentation/d/1REISRX2a9dt76wby0hn_ntIRVl_nI4-1/edit?usp=sharing&amp;ouid=117917823223915543509&amp;rtpof=true&amp;sd=true"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 08
                <a
                  href="https://docs.google.com/presentation/d/1jOddwBSxs5gy9h7YvwtIOJjva1hYAZVH/edit?usp=sharing&amp;ouid=117917823223915543509&amp;rtpof=true&amp;sd=true"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 09
                <a
                  href="https://drive.google.com/file/d/1vVSxQnG7ibQXL36PHKHsVl0g0ELpb4S3/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 10
                <a
                  href="https://drive.google.com/file/d/1ObRS2_-ERBcHszPf-_5wXLqLFwYK1khI/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 11
                <a
                  href="https://drive.google.com/file/d/1TtZvRu-eYG5pnAlv294SzIQq4HOfozSL/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 12
                <a
                  href="https://drive.google.com/file/d/1BYvpfR6HuVs2OAGAvvyNYvWcDn7tbnp6/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 13
                <a
                  href="https://drive.google.com/file/d/1nG2PdNVZFbRGMhzmPIin4gx9axPgLxa-/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 14
                <a
                  href="https://drive.google.com/file/d/1Iwlj5qmGMKJcsNpAHpp_vJMjD3UQDwZ4/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 15
                <a
                  href="https://drive.google.com/file/d/1CSNOxmj8OehWC0J6FR9wmzwzX3KpGj5D/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 16
                <a
                  href="https://drive.google.com/file/d/1jOFCwCyKuF4mZIvfXx0BzbNsTXc74mmd/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 17
                <a
                  href="https://drive.google.com/file/d/1j-B0mltnUv6YiDT9jsdaUniz9ul4scAk/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 18
                <a
                  href="https://drive.google.com/file/d/1QQxmSQAzoBhpUkyDEYD1drkY7CouYyne/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 19
                <a
                  href="https://drive.google.com/file/d/18vMwGPhw5ald9NKPuetCdIlZZVTd5OyJ/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 20
                <a
                  href="https://drive.google.com/file/d/1XxbtgG79bq2JbMa_bs7o3kxQodVjeKtJ/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 21
                <a
                  href="https://drive.google.com/file/d/1h3YkZnVwLryhrJypImZIBy1ZtIzxgrFz/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 22
                <a
                  href="https://drive.google.com/file/d/1fbtscfiXBjwI1wXZzYeNUQm00yP9r631/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 23
                <a
                  href="https://drive.google.com/file/d/1ee15VlN8WceFmsMho8LVrJO-CE9sp-cz/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 24
                <a
                  href="https://drive.google.com/file/d/1zRQxn-f4YnnDDxXNv9H_h6jy7a7hj5zX/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 25
                <a
                  href="https://drive.google.com/file/d/1EkReMvDuxRj8ayNfSztNTti_7a8q7VFv/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 26
                <a
                  href="https://drive.google.com/file/d/1uxCUlkrU0z1dTevF9ivjzjGHE6iIuGea/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 27
                <a
                  href="https://drive.google.com/file/d/1fCXub5jLf665A-yO-Y8hhs5gkExDtMbd/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>

              <li>
                Aula 28
                <a
                  href="https://drive.google.com/file/d/1tvPciEh4WL5jp4TYb69NTYPzEjaaceua/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pptx)
                </a>
              </li>
            </ul>

            <li>Lista de problemas</li>
            <ul>
              <li>
                Lista de problemas selecionados do volume 2 do Halliday
                <a
                  href="https://drive.google.com/file/d/1aG2LC0Og8KRlodHTuofU8jh5pTCu8e4p/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>
            </ul>

            <li>Provas e suas soluções</li>
            <ul>
              <li>
                Prova 01 - 2019.1
                <a
                  href="https://drive.google.com/file/d/1Bf0w8JhFP459_Z0RIYiYYmJfox2yGyhK/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 02 - 2019.1
                <a
                  href="https://drive.google.com/file/d/1nC7di20nVGQckE7UJmoTJ3SVBM07_rMJ/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 03 - 2019.1
                <a
                  href="https://drive.google.com/file/d/10X_QMx0ajL4lK7MpqDEZgSoixonWgugs/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova de segunda chamada - 2019.1
                <a
                  href="https://drive.google.com/file/d/12JSW8ZLqoMufOuXgb8nbkXjK3ZXlCK5_/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova final - 2019.1
                <a
                  href="https://drive.google.com/file/d/1cftrbHhhHsxWEIBNY00eTD3Q3SEqJcdI/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 01 - 2019.2
                <a
                  href="https://drive.google.com/file/d/1dRXIJ3iAvv2HsTiMp1Duazu61IMQAm39/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 02 - 2019.2
                <a
                  href="https://drive.google.com/file/d/1q2yTfZUyiJG4w72IgVHAiP_qjEeUIbUg/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 03 - 2019.2
                <a
                  href="https://drive.google.com/file/d/1K8THKH8OcVzlruIVX6pIoo-Gh4vA6Ajm/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova de segunda chamada - 2019.2
                <a
                  href="https://drive.google.com/file/d/1NmKY4YJcqdZHhpSWPD640BlM5aWoDdLo/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova final - 2019.2
                <a
                  href="https://drive.google.com/file/d/1TfPF_u7gmsXYcDiYa8twGkF-Dd-nsyC_/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 01 - 2022.2
                <a
                  href="https://drive.google.com/file/d/1v3M38TQeIStnA0mifCzMXLhmHwkMn2PR/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 02 - 2022.2
                <a
                  href="https://drive.google.com/file/d/1GZlwWF2I77uWoExk4V3mBmIxysvKSVZl/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 03 - 2022.2
                <a
                  href="https://drive.google.com/file/d/1N7OA2Yz6F7joctcq82jm3saQ3wse8V_l/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova de segunda chamada - 2022.2
                <a
                  href="https://drive.google.com/file/d/1MK4yFzelCwWnMsn1wvoNX-QHhPjM9ftc/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova final - 2022.2
                <a
                  href="https://drive.google.com/file/d/1W3P89OL_kKuTqE7MAoDEVFAMs98XJKea/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 01 - 2023.1
                <a
                  href="https://drive.google.com/file/d/1bnYdMESoAyDvQWrnMvdZ-hdS0PCZU_yc/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 02 - 2023.1
                <a
                  href="https://drive.google.com/file/d/1wVAX7D1NjQ0L-W8SMTMWZSas6gg7eBC2/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 03 - 2023.1
                <a
                  href="https://drive.google.com/file/d/1EgCaZP-ewyo2YzmoWCm3wP3XNf2H1Mis/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova de segunda chamada - 2023.1
                <a
                  href="https://drive.google.com/file/d/1IOy_rNrdoQ3ELNu-cLGmfE7D3F_GFQQk/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova final - 2023.1
                <a
                  href="https://drive.google.com/file/d/1qXfU7x7sGcOAD5Pv7goavzWRLCAQ136f/view"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 01 - 2023.2
                <a
                  href="https://drive.google.com/file/d/15LP2AxWR5laUp_gJUJEMOPWO1yHat_gB/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova 02 - 2023.2
                <a
                  href="https://drive.google.com/file/d/1x3xPkv9SEQNoTZSGHeJAod2nVa0_kUXg/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova de segunda chamada - 2023.2
                <a
                  href="https://drive.google.com/file/d/19Ala0uAUIUWd2zoFJDiYxuiL8Knvbols/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>

              <li>
                Prova final - 2023.2
                <a
                  href="https://drive.google.com/file/d/1r48ulKVK5xP0tM8w3wDjsIlvn6EPm7SS/view?usp=sharing"
                  target="_blank"
                  type="application/pdf"
                >
                  (pdf)
                </a>
              </li>
            </ul>
          </ul>
        </section>
      </div>
      <a
        className="u-url"
        href="/2024/04/16/F%C3%ADsica-Geral-2.html"
        hidden
      ></a>
    </article>
  );
};

export default Fis2Course;
